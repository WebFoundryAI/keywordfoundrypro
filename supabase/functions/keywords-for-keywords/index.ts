...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (keywords.length > 20) {
      return new Response(JSON.stringify({ error: 'Maximum 20 keywords allowed' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }

    console.log('Keywords for keywords request:', { keywords: keywords.length, location_name, language_name })
    ...
    const results = task.result?.[0]?.keywords || []
